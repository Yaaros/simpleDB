...
public class BufferPool {
    ...
    private final int numPages; // 缓存页面的最大数量
    private final ConcurrentHashMap<PageId, Page> pages; //缓存手段
    ...
    public Page getPage(TransactionId tid, PageId pid, Permissions perm)
        ...
        if(perm.permLevel>-1){
            ...
            if(p==null){
                // Page is not in buffer pool, fetch it from the disk
                if (pages.size() >= numPages) {
                    evictPage(); // Evict a page if the buffer pool is full
                }
                // Assume that DbFile is accessible and has a method to read a page from disk
